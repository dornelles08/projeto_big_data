...
class Th(Thread):
    ...
    def run(self):
        ...
        file.write(
            f"{ctime()} - {self.id} - Quantidade de Links: {len(links)}\n")
        print(f"{ctime()} - {self.id} - Quantidade de Links: {len(links)}")

        count = 0
        for l in links:
            count += 1
            try:
                file.write(f"{ctime()} - {self.id} - Link {count}\n")
        ...
        file.write(
            f"{ctime()} - {self.id} - Quantidade de Carros Coletados: {len(carros)}\n")

        if(len(errors) > 0):
            file.write(
                f"{ctime()} - {self.id} - Quantidade de Erros: {len(errors)}\n")
            insert_many(errors, 'erros')

        fim = time()
        file.write(
            f"{ctime()} - {self.id} - Tempo para coletar os dados {round(fim-inicio, 2)} s\n")
        print(
            f"{ctime()} - {self.id} - Tempo para coletar os dados {round(fim-inicio, 2)} s")
        file.write(
            f"{ctime()} - {self.id} - Tempo médio para coletar os dados {round(fim-inicio, 2)/len(links)} s\n")
